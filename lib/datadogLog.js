...
var _process$env$DATADOG_ = _process$env.DATADOG_CURL;
const DATADOG_CURL = _process$env$DATADOG_ === undefined ? 'curl' : _process$env$DATADOG_;
var _process$env$DATADOG_2 = _process$env.DATADOG_HOST;
const DATADOG_HOST = _process$env$DATADOG_2 === undefined ? 'app.datadoghq.com' : _process$env$DATADOG_2;
var _process$env$DATADOG_3 = _process$env.DATADOG_API_KEY;
const DATADOG_API_KEY = _process$env$DATADOG_3 === undefined ? isRequried('DATADOG_API_KEY') : _process$env$DATADOG_3;


const datadog = (path, data) => spawn(DATADOG_CURL, ['--silent', '--show-error', '--request', 'POST', '--header', 'Content-Type: application/json', '--data', data, `https://${ DATADOG_HOST }/api/v1/${ path }?api_key=${ DATADOG_API_KEY }`]);
